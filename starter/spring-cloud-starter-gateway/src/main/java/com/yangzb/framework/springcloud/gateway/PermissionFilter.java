package com.yangzb.framework.springcloud.gateway;
...
import com.yangzb.framework.springcloud.gateway.service.PermissionService;
import com.yangzb.framework.springcloud.gateway.service.UserService;
...
public class PermissionFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (StringUtils.isEmpty(token)) {
            throw new AuthenticationException(Constants.ERROR_8000, "认证失败，请重新登录！");
        }
        ...
        if (!permissionService.hasPermission(systemUser.getPrivileges(), exchange.getRequest().getURI().getPath())) {
            throw new ForbiddenException(Constants.ERROR_8001, "您无权访问服务！");
        }
