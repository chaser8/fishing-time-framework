...
import com.yangzb.framework.common.web.exception.BusiException;
...
public class ControllerAdvice {
    ...
    public Response errorHandler(Exception ex)throws Exception {
        ...
        responseBodyError.setMessage(ex.getMessage());
    ...
    public org.springframework.http.ResponseEntity errorHandler(HttpMessageNotReadableException ex) {
        ...
        ResponseBodyError responseBodyError = Constants.ERROR_9997;
        responseBodyError.setMessage(message);
        return  new org.springframework.http.ResponseEntity(responseBodyError, Response.PROBLEM_HEADERS,HttpStatus.BAD_REQUEST);
    }
    ...
    public Response errorHandler(AuthenticationException ex) {
    ...
    public Response busiExceptionHandler(BusiException ex) {
        Response<ResponseBodyError> response = ex.getResponse();
        if(!StrUtil.isEmpty(ex.getMessage())){
            response.getBody().setMessage(ex.getMessage());
        }
    ...
    public org.springframework.http.ResponseEntity bindExceptionHandler(Exception ex) {
        ...
        ResponseBodyError responseBodyError = Constants.ERROR_9997.setMessage(message.toString());
        return  new org.springframework.http.ResponseEntity(responseBodyError, Response.PROBLEM_HEADERS,HttpStatus.BAD_REQUEST);
    }
